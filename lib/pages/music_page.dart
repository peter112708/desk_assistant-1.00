...
class MusicController {
  ...
  // Stream subscriptions — stored so they can be cancelled
  ...
  MusicController() {
    // Position changes — merged into a single listener
    _posSub = _audioPlayer.onPositionChanged.listen((pos) async {
      if (_isDisposed) return;
      _positionController.add(pos);

      // Fallback: fetch duration if not yet available
      if (_currentDuration.inSeconds == 0) {
        final platformDuration = await _audioPlayer.getDuration();
        if (!_isDisposed && platformDuration != null && platformDuration.inSeconds > 0) {
...
class _MusicPageState extends State<MusicPage> with AutomaticKeepAliveClientMixin {
  ...
  final List<Map<String, String>> _searchResults = [
    {'title': '七里臭 (测试音源1)', 'artist': '周杰伦', 'url': 'https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3'},
    {'title': '阴天 (测试音源2)', 'artist': '周杰伦', 'url': 'https://www.soundhelix.com/examples/mp3/SoundHelix-Song-2.mp3'},
    {'title': '告白雨伞 (测试音源3)', 'artist': '周杰伦', 'url': 'https://www.soundhelix.com/examples/mp3/SoundHelix-Song-3.mp3'},
  ];
...
// 📥 从本地硬盘恢复爱心歌单
  ...
  void dispose() {
    ...
    _controller.dispose();
    _dio.close();
    // Kill the Python process tree (use taskkill /T on Windows to avoid orphans)
